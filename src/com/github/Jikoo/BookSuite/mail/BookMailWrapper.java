...
public class BookMailWrapper {
	 ...
	 * Author: Sender
	 * 
	 * Page 1: To Page 2: (optional, though must be blank for sending items)
	 * Associated Message Page 3: (optional) Item Names Page 4: Generated by us.
	 * Anything the sender puts here is erased, and we populate with data. Apon
	 * reading of the letter, this data is accessed by us, to check if the user
	 * has send items If those items fit in the inventory, add them, remove this
	 * page, and add the items, else show message to user telling them that the
	 * items cannot be added and they should increase their inventory free space
	 */
	...
	private Inventory getInventoryBySerializedIDs(String serial) {
		...
		while ((line = stringParser.nextLine()) != null && ++count < 5) {
			...
			for (int iterator = 2; iterator < iai.length; iterator++) {
				...
				Enchantment e = Enchantment.getById(Integer
						.parseInt(enchantData[0]));
